...
import { createFolderInDrive, createDatabaseFiles } from '../services/driveService';
...
const CompanySetup: React.FC<{ onSetupComplete: (data: CompanyData) => void; token: string }> = ({ onSetupComplete, token }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const folderId = await createFolderInDrive(formData.companyName, formData.drivePath, token);

      // Criar arquivos de banco de dados
      await createDatabaseFiles(folderId, formData, token);

      // Salvar dados de configuração (localStorage ou backend)
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: '15px' }}>
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="Digite o contato (telefone ou email)"
            style={{ width: '100%', padding: '8px', marginTop: '5px' }}
          />
        </div>

        {/* Caminho do Diretório no Drive */}
        <div style={{ marginBottom: '15px' }}>
